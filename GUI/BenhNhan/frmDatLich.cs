...
namespace AppDatLichKham.GUI.BenhNhan
{
    public partial class frmDatLich : Form
    {
        ...
        private void siticoneComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                List<BacSi> danhsachbacsi = BacSiDAL.Instance.GetBacSiByChuyenKhoa(selectedValue);
                siticoneComboBox2.DataSource = danhsachbacsi;
                siticoneComboBox2.DisplayMember = "HoTen";
                siticoneComboBox2.ValueMember = "BacSiID";
                pnlBtn.Visible=false;
        ...
        {
            BacSi bs = BacSiDAL.Instance.GetBacSiByID(Convert.ToInt32(txtBacsiid.Text));
            lblTenbacsi.Text = bs.HoTen;
            lblNgayhen.Text = dtk.SelectedRanges[0].StartDate.Date.ToString("dd-MM-yyy");
        ...
        {
            if (DatLichBLL.Instance.ThemDatLich(txtBacsiid.Text, StaticThing.idBenhNhanTaiKhoan, txtTenBenhNhan.Text, Nam.Checked, txtSdt.Text, txtDiachi.Text, txtghichu.Text, txtNgay.Text, txtGioHen.Text))
            {
        ...
        private void dtk_MouseDown(object sender, MouseEventArgs e)
        {
        }
        private frmDatLichnew parentForm;

        public frmDatLich(frmDatLichnew frmCha)
        {
            InitializeComponent();
            parentForm = frmCha;
        }
